...
export function getConfig() {
    if (!configLocation) {
        configLocation = findUp.sync(FILE_NAME);
    }
    if (!configLocation) {
        console.error(`${FILE_NAME} not found. Please place it in the root of your project with { "dataDir": "./db/data" } (pointing to your data folder) as the contents.`);
        process.exit(1);
    }
    try {

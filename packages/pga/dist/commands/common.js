...
import { createExtensions, createRelations, createTables, seedTables, } from '@ffwd/db-util';
export async function createCmd(tables, cmdObj) {
    const dirs = getAllDirectories();
    await createTables(dirs.schemas, tables, cmdObj);
    process.exit(0);
}
export async function seedCmd(tables, cmdObj) {
    const dirs = getAllDirectories();
    await seedTables(dirs.seeds, tables, cmdObj);
    process.exit(0);
}
export async function extCmd(extensions, cmdObj) {
    const dirs = getAllDirectories();
    await createExtensions(dirs.extensions, extensions, cmdObj);
    process.exit(0);
}
export async function relCmd(tables, cmdObj) {
    const dirs = getAllDirectories();

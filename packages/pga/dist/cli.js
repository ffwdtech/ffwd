...
program
    .command('init [folder]')
    .description(`Initialise a new project- creates the database skeleton folders, tests bootstrap folder and ${FILE_NAME}. Pass an optional folder to skip prompting it.`)
    .action(initCmd);
program
    .command('list')
...
program
    ...
    .alias('ext')
    .alias('e')
    .option('-a, --all', 'Create all extensions available in the extensions directory.')
    .description('Creates postgres extensions')
    .action(extCmd);
program
    .command('create [tables...]')
    .alias('c')
    .option('-a, --all', 'Create all the tables defined in the schemas folder. Will not try to create duplicate tables.')
...
program
    ...
    .alias('rel')
    .alias('r')
    .option('-a, --all', 'Create all relations available in the relations directory.')
    .description('Create table relations')
    .action(relCmd);
program
    .command('seed [tables...]')
    .alias('s')
    .option('-a, --all', 'Run all insert statements for all available tables in the seeds directory. Executing this on an existing database can create duplicate data.')
...
program
    ...
    .description('Remove a row (or all rows) from a table. If you have a string-based enum table, just use the primary key value.')
    .action(rmCmd);
program
    .command('drop <tables...>')
    .alias('d')
...
program
    ...
    .option('-f --force <number>', 'Force update the migration number in the database without processing any migration files')

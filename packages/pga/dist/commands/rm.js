...
export async function rmCmd(table, keys, cmdObj) {
    ...
    if (cmdObj.all) {
        ...
        if (!keys) {
            console.log('No keys provided, if you want to clear the whole table use --all');
            process.exit(0);
        }
    }
    ...
    let idField;
    if (rows.find((row) => row.column_name === 'id')) {
        idField = 'id';
    }
    else if (rows.find((row) => row.column_name === 'value')) {
        idField = 'value';
    }
    if (!idField) {
